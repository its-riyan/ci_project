    ...
    function generate_category_url($category)
    {
        ...
        if (!empty($category)) {
            if ($category->parent_id == 0) {
                return lang_base_url() . "product-category" . "/" . $category->slug;
            } else {
...
//generate brand url
if (!function_exists('generate_brand_url')) {
    function generate_brand_url($brand)
    {

        if (!empty($brand)) {
            if ($brand->parent_id == 0) {
                return lang_base_url() . "product-brand" . "/" . $brand->slug;
            } else {
                return lang_base_url() . "product-brand" . "/" . $brand->parent_slug . "/" . $brand->slug;
            }
        }
    }
}
...
if (!function_exists('generate_product_url')) {
    ...
    {
        if (!empty($product)) {
            return lang_base_url() . get_route("product", true) . $product->slug;
        }
    }
}
...
if (!function_exists('generate_product_url_by_slug')) {
    ...
    {
        if (!empty($slug)) {
            return lang_base_url() . get_route("product", true) . $slug;
        }
    }
}
...
//get lifeline content image
if (!function_exists('get_lifeline_file_manager_image')) {
    function get_lifeline_file_manager_image($image)
    {
        $path = base_url() . 'assets/img/no-image.jpg';
        $ci =& get_instance();
        if (!empty($image)) {
...
if (!function_exists('get_products_count_by_brand')) {
    function get_products_count_by_brand($brand_id)
    {
        $ci =& get_instance();
        return $ci->product_model->get_products_count_by_brand($brand_id);
    }
}

//get product count by subbrand
if (!function_exists('get_products_count_by_subbrand')) {
    function get_products_count_by_subbrand($brand_id)
    {
        $ci =& get_instance();
        return $ci->product_model->get_products_count_by_subbrand($brand_id);
    }
}

//get dropdown brand id
if (!function_exists('get_dropdown_brand_id')) {
    function get_dropdown_brand_id()
    {
        $ci =& get_instance();
        $brand_id = 0;
        $brand_ids = $ci->input->post('brand_id');
        if (!empty($brand_ids)) {
...
//get lifeline image url
if (!function_exists('get_lifeline_image_url')) {
    function get_lifeline_image_url($post, $size_name)
    {
        if ($post->storage == "aws_s3") {
            $ci =& get_instance();
            return $ci->aws_base_url . $post->$size_name;
...
if (!function_exists('get_cart_product_count')) {
    ...
    {
        $ci =& get_instance();
        if (!empty($ci->session->userdata('kss_shopping_cart'))) {
            return @count($ci->session->userdata('kss_shopping_cart'));
        } else {
            return 0;
        }
    }
...
if (!function_exists('get_cart_discount_coupon')) {
    ...
    {
        $ci =& get_instance();
        if (!empty($ci->session->userdata('kss_cart_coupon_code'))) {
            return $ci->session->userdata('kss_cart_coupon_code');
        }
    }
}
...
if (!function_exists('get_cart_customer_data')) {
    function get_cart_customer_data()
    {
        ...
        if ($ci->auth_check) {
            ...
            $cart_shipping = $ci->session->userdata('kss_cart_shipping');
...
if (!function_exists('check_newsletter_modal')) {
    ...
    {
        if (!$ci->auth_check && $ci->general_settings->newsletter_status == 1 && $ci->general_settings->newsletter_popup == 1 && empty($_COOKIE['kss_news_mdl'])) {
            setcookie('kss_news_mdl', '1', time() + (86400 * 30), "/");
            return true;
        }
        return false;
    }
