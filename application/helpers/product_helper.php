...
//get parent brands
if (!function_exists('get_parent_brands')) {
    function get_parent_brands()
    {
        $ci =& get_instance();
        if (!empty($ci->brands_array)) {
            if (isset($ci->brands_array[0])) {
                return $ci->brands_array[0];
            }
        }
        return null;
    }
...
//get subbrand
if (!function_exists('get_subbrands')) {
    function get_subbrands($parent_id)
    {
        $ci =& get_instance();
        return $ci->brand_model->get_subbrands($parent_id);
    }
}


//get brands json
if (!function_exists('get_brands_json')) {
    function get_brands_json($lang_id)
    {
        $ci =& get_instance();
        return $ci->brand_model->get_brands_json($lang_id);
    }
}
...
if (!function_exists('brand_name')) {
    function brand_name($brand)
    {
        if (!empty($brand)) {
            if (!empty($brand->name)) {
                return html_escape($brand->name);
            } else {
                if (!empty($brand->second_name)) {
                    return html_escape($brand->second_name);
                }
            }
        }
        return "";
...
if (!function_exists('get_brand_image_url')) {
    function get_brand_image_url($brand)
    {
        if ($brand->storage == "aws_s3") {
            $ci =& get_instance();
            return $ci->aws_base_url . $brand->image;
        } else {
            return base_url() . $brand->image;
        }
    }
}

//get parent brands tree
if (!function_exists('get_parent_brands_tree')) {
    function get_parent_brands_tree($brand, $only_visible = true)
    {
        $ci =& get_instance();
        return $ci->brand_model->get_parent_brands_tree($brand, $only_visible);
    }
}
        ...
        if (!empty($product)) {
            ...
            if ($product->listing_type == 'sell_on_site' || $product->listing_type == 'license_key') {
                if ($product->is_free_product != 1) {
                    ...
                    $data->button = '<button class="ps-btn ps-btn--primary"' . $disabled . '><i class="fa fa-plus"></i> ' . trans("quotation") . '</button>';
                }
            } elseif ($product->listing_type == 'bidding') {
                $data->add_to_cart_url = base_url() . 'request-quote';
                $data->button = '<button class="ps-btn ps-btn--primary"' . $disabled . '>' . trans("request_a_quote") . '</button>';
                if (!$ci->auth_check && $product->listing_type == 'bidding') {
                    $data->button = '<button type="button" data-toggle="modal" data-target="#loginModal" class="ps-btn ps-btn--primary"' . $disabled . '>' . trans("request_a_quote") . '</button>';
                }
            } else {
                if ($ci->auth_check) {
                    $data->button = '<button type="button" class="ps-btn ps-btn--primary" data-toggle="modal" data-target="#messageModal">' . trans("contact_seller") . '</button>';
                } else {
                    $data->button = '<button type="button" class="ps-btn ps-btn--primary" data-toggle="modal" data-target="#loginModal">' . trans("contact_seller") . '</button>';
                }
                if (!empty($product->external_link)) {
                    $data->button = '<a href="' . $product->external_link . '" class="ps-btn ps-btn--primary" target="_blank" rel="nofollow">' . trans("buy_now") . '</a>';
                }
            }
        }
        return $data;
...
if (!function_exists('get_product_item_image')) {
    function get_product_item_image($product, $get_second = false)
    {
        ...
        return base_url() . '@static/lazy/product-list-thumbnail.png';
    }
}
...
if (!function_exists('get_index_brands_products')) {
    function get_index_brands_products($ci, $brands)
    {
        return $ci->product_model->get_index_brands_products($brands);
    }
}
...
if (!function_exists('is_product_in_wishlist')) {
    function is_product_in_wishlist($product)
    {
        ...
        if ($ci->auth_check) {
            ...
            $wishlist = $ci->session->userdata('kss_guest_wishlist');
            if (!empty($wishlist)) {
