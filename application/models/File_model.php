...
class File_model extends CI_Model
{
    ...
    public function upload_image_session()
    {
        ...
        if (!empty($temp_path)) {
            ...
            $kss_images = $this->get_sess_product_images_array();
            $item = new stdClass();
            ...
            $item->is_main = 0;
            $item->file_time = time();
            array_push($kss_images, $item);
            $this->set_sess_product_images_array($kss_images);
            $this->upload_model->delete_temp_image($temp_path);
        }
    }
    ...
    public function add_product_images($product_id)
    {
        $kss_images = $this->get_sess_product_images_array();
        if (!empty($kss_images)) {
            foreach ($kss_images as $kss_image) {
                if (!empty($kss_image)) {
                    $image_storage = "local";
                    $this->load->model('upload_model');
                    ...
                    if ($this->storage_settings->storage == "aws_s3") {
                        ...
                        $this->load->model("aws_model");
                        //move default image
                        $this->aws_model->put_product_object($directory . $kss_image->img_default, FCPATH . "uploads/temp/" . $kss_image->img_default);
                        delete_file_from_server("uploads/temp/" . $kss_image->img_default);
                        //move big image
                        $this->aws_model->put_product_object($directory . $kss_image->img_big, FCPATH . "uploads/temp/" . $kss_image->img_big);
                        delete_file_from_server("uploads/temp/" . $kss_image->img_big);
                        //move small image
                        $this->aws_model->put_product_object($directory . $kss_image->img_small, FCPATH . "uploads/temp/" . $kss_image->img_small);
                        delete_file_from_server("uploads/temp/" . $kss_image->img_small);
                    } else {
                        //move default image
                        copy(FCPATH . "uploads/temp/" . $kss_image->img_default, FCPATH . "uploads/images/" . $directory . $kss_image->img_default);
                        delete_file_from_server("uploads/temp/" . $kss_image->img_default);
                        //move big image
                        copy(FCPATH . "uploads/temp/" . $kss_image->img_big, FCPATH . "uploads/images/" . $directory . $kss_image->img_big);
                        delete_file_from_server("uploads/temp/" . $kss_image->img_big);
                        //move small image
                        copy(FCPATH . "uploads/temp/" . $kss_image->img_small, FCPATH . "uploads/images/" . $directory . $kss_image->img_small);
                        delete_file_from_server("uploads/temp/" . $kss_image->img_small);
                    }
                    ...
                    $data = array(
                        'product_id' => $product_id,
                        'image_default' => $directory . $kss_image->img_default,
                        'image_big' => $directory . $kss_image->img_big,
                        'image_small' => $directory . $kss_image->img_small,
                        'is_main' => $kss_image->is_main,
    ...
    public function set_sess_product_images_array($kss_images)
    {
        $this->session->set_userdata('kss_product_images', $kss_images);
    }
    ...
    {
        $kss_images = array();
        if (!empty($this->session->userdata('kss_product_images'))) {
            $kss_images = $this->session->userdata('kss_product_images');
        }
        if (!empty($kss_images)) {
            usort($kss_images, function ($a, $b) {
    ...
    {
        if (!empty($this->session->userdata('kss_product_images'))) {
            $this->session->unset_userdata('kss_product_images');
        }
    }
        ...
        if (!empty($kss_images)) {
            foreach ($kss_images as $kss_image) {
                if ($kss_image->file_id == $file_id) {
                    ...
                    $item = new stdClass();
                    $item->img_default = $kss_image->img_default;
                    $item->img_big = $kss_image->img_big;
                    $item->img_small = $kss_image->img_small;
                    $item->file_id = $kss_image->file_id;
                    $item->is_main = $kss_image->is_main;
                    $item->file_time = $kss_image->file_time;
                    array_push($kss_images_new, $item);
                }
            }
        }
        unset($kss_images);
        $this->set_sess_product_images_array($kss_images_new);
    }
    ...
    */
    ...
    //upload image
    public function upload_lifeline_image()
    {
        $this->load->model('upload_model');
        $temp_path = $this->upload_model->upload_temp_image('file');
        if (!empty($temp_path)) {
            $data = array(
                'image_path' => $this->upload_model->lifeline_content_image_upload($temp_path),
                'image_path_thumb' => $this->upload_model->lifeline_cover_small_upload($temp_path),
                'storage' => "local",
                'user_id' => $this->auth_user->id
            );
            //move to s3
            if ($this->storage_settings->storage == "aws_s3") {
                ...
                //move images
                if (!empty($data["image_path"])) {
                    $this->aws_model->put_lifeline_object($data["image_path"], FCPATH . $data["image_path"]);
                    delete_file_from_server($data["image_path"]);
                }
                if (!empty($data["image_path_thumb"])) {
                    $this->aws_model->put_lifeline_object($data["image_path_thumb"], FCPATH . $data["image_path_thumb"]);
                    delete_file_from_server($data["image_path_thumb"]);
                }
            }
            @$this->db->close();
            @$this->db->initialize();
            $this->db->insert('lifeline_images', $data);
            $this->upload_model->delete_temp_image($temp_path);
        }
    }

    //get lifeline images
    public function get_lifeline_images()
    {
        $this->db->order_by('id', 'DESC');
        $query = $this->db->get('lifeline_images');
        return $query->result();
    }

    //get lifeline image
    public function get_lifeline_image($file_id)
    {
        $this->db->where('id', $file_id);
        $query = $this->db->get('lifeline_images');
        return $query->row();
    ...
    //delete lifeline image
    public function delete_lifeline_image($file_id)
    {
        $image = $this->get_lifeline_image($file_id);
        if (!empty($image)) {
            if ($image->storage == "aws_s3") {
                $this->load->model("aws_model");
                $this->aws_model->delete_lifeline_object($image->image_path);
                $this->aws_model->delete_lifeline_object($image->image_path_thumb);
            } else {
                delete_file_from_server($image->image_path);
                delete_file_from_server($image->image_path_thumb);
            }
            $this->db->where('id', $image->id);
            $this->db->delete('lifeline_images');
        }
    }
