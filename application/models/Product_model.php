...
class Product_model extends CI_Model
{
    ...
    public function filter_products($query_string_array = null, $category_ids = null, $custom_filters = null, $user_id = null)
    {
        ...
        } elseif (!empty($sort) && $sort == "most_previous") {
            $this->db->order_by('products.id');

        } elseif (!empty($sort) && $sort == "alphabetical_order") {
            $this->db->order_by('products.slug');
    ...
    public function search_products($search)
    {
        if (!empty($search)) {
            ...
            $array_like = array();
            $this->build_query();
            $this->db->join('product_details', 'product_details.product_id = products.id', 'brands', 'brands.title_meta_tag = products.id');
            $this->db->where('product_details.lang_id', clean_number($this->selected_lang->id));
            $this->db->group_start();
            foreach ($array as $item) {
                if (strlen($item) > 1) {
            ...
            $this->db->or_like('products.sku', clean_slug($search));
            $this->db->group_end();
            $this->db->order_by('products.is_promoted', 'DESC')->limit(150);
    ...
    //get index brands products
    public function get_index_brands_products($brands)
    {
        $limit = 6;
        $lc_key = get_location_cache_key($this);
        $key = "index_brand_products_lang_" . $this->selected_lang->id;
        $result_cache = get_cached_data($this, $key, "pr");
        if (!empty($result_cache)) {
            if (!empty($result_cache[$lc_key])) {
                return $result_cache[$lc_key];
        ...
        if (!empty($brands)) {
            foreach ($brands as $brand) {
                if ($brand->show_subbrand_products == 1) {
                    $this->build_query();
                    $this->db->group_start();
                    $this->db->where("products.brand_id IN (SELECT id FROM brands WHERE FIND_IN_SET(" . clean_number($brand->id) . ", brands.parent_tree))");
                    $this->db->or_where("products.brand_id", clean_number($brand->id));
                    $this->db->group_end();
                    $this->db->order_by('products.price', 'DESC')->limit($limit);
                    $products_array[$brand->id] = $this->db->get('products')->result();
                } else {
                    $this->build_query();
                    $this->db->where_in('products.brand_id', clean_number($brand->id), FALSE);
                    $this->db->order_by('products.price', 'DESC')->limit($limit);
                    $products_array[$brand->id] = $this->db->get('products')->result();
                }
            }
    ...
    public function get_paginated_filtered_products($query_string_array, $category_ids, $custom_filters, $per_page, $offset)
    {
        ...
        $this->db->limit(clean_number(40), clean_number($offset));
        return $this->db->get('products')->result();
    }
    ...
    //get paginated filtered products
    public function get_paginated_filtered_brands($query_string_array, $brand_ids, $custom_filters, $per_page, $offset)
    {
        $this->filter_products($query_string_array, $brand_ids, $custom_filters);
        $this->db->limit(clean_number($per_page), clean_number($offset));
        return $this->db->get('products')->result();
    }

    //get paginated filtered products count
    public function get_paginated_filtered_brands_count($query_string_array, $brand_ids, $custom_filters)
    {
        $this->filter_products($query_string_array, $brand_ids, $custom_filters);
        return $this->db->count_all_results('products');
    }
    ...
    public function get_brands_count_by_brand($brand_ids)
    {
        return $this->db->where('products.brand_id', clean_number($brand_ids))->count_all_results('products');
    }
    ...
    public function get_related_products_by_brand($product_id, $brand_id)
    {
        $lc_key = get_location_cache_key($this);
        $lc_key = "pr" . $product_id . "_" . $lc_key;
        $key = "related_products_brand_lang_" . $this->selected_lang->id;
        $result_cache = get_cached_data($this, $key, "pr");
        if (!empty($result_cache)) {
            if (!empty($result_cache[$lc_key])) {
                return $result_cache[$lc_key];
        ...
        $this->build_query();
        $result = $this->db->where('products.brand_id', clean_number($brand_id))->where('products.id !=', clean_number($product_id))->order_by('rand()')->limit(10)->get('products')->result();
    ...
    public function filter_user_products($list_type)
    {
        ...
        $brand = clean_number(input_get('brand'));
        $subbrand = clean_number(input_get('subbrand'));
        ...
        if (!empty($brand_id)) {
            $brand_ids = $this->brand_model->get_subbrands_tree_ids($brand_id, true, true);;
        }
        ...
        if (!empty($brand_ids)) {
            $this->db->where_in("products.brand_id", $brand_ids, FALSE);
        }
    ...
    {
        $wishlist = $this->session->userdata('kss_guest_wishlist');
    ...
    public function add_remove_wishlist($product_id)
    {
        if ($this->auth_check) {
            ...
            if ($this->is_product_in_wishlist($product_id)) {
                $wishlist = array();
                if (!empty($this->session->userdata('kss_guest_wishlist'))) {
                    $wishlist = $this->session->userdata('kss_guest_wishlist');
                }
                $new = array();
                if (!empty($wishlist)) {
                ...
                $this->session->set_userdata('kss_guest_wishlist', $new);
            } else {
                $wishlist = array();
                if (!empty($this->session->userdata('kss_guest_wishlist'))) {
                    $wishlist = $this->session->userdata('kss_guest_wishlist');
                ...
                $this->session->set_userdata('kss_guest_wishlist', $wishlist);
            }
        }
    }
    ...
    {
        if (!empty($product)) {
            if (!isset($_COOKIE['kss_product_' . $product->id])) {
                //increase hit
                setcookie("kss_product_" . $product->id, '1', time() + (86400 * 300), "/");
                $data = array(
